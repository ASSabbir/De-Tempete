...
export default function Publications() {
  ...
  const handleSave = async () => {
    ...
    setError('');
    try {
      const { data } = editing ? await API.put(`/publications/${editing}`, form) : await API.post('/publications', form);
      if (editing) {
  ...
  const handlePublish = async (id) => {
    try {
      const { data } = await API.patch(`/publications/${id}/status`, { status: 'published' });
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 32, width: '100%', maxWidth: 480, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <input value={form.fileFormat} onChange={e => setForm(p => ({ ...p, fileFormat: e.target.value }))} style={inputStyle} />
            </div>

            <div style={{ marginBottom: 16 }}>
              <label style={labelStyle}>Download URL</label>
              <input value={form.downloadUrl} onChange={e => setForm(p => ({ ...p, downloadUrl: e.target.value }))} style={inputStyle} />
            </div>
